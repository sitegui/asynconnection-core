...
FrameEncoder.prototype.doHandshake = function (user, password, calls, messages) {
	this._connection.sendFrame(FrameEncoder._handshakeType.writeIntoBuffer({
		version: 1,
		auth: {
			user: user,
			password: password
...
FrameEncoder.prototype.doAnswerHandshake = function (ok) {
	this._connection.sendFrame(FrameEncoder._handshakeAnswerType.writeIntoBuffer({
		ok: ok
	}))
}
...
FrameEncoder.prototype.readHandshake = function (frame) {
	return FrameEncoder._handshakeType.read(new ReadState(frame))
}
...
FrameEncoder.prototype.readHandshakeAnswer = function (frame) {
	return FrameEncoder._handshakeAnswerType.read(new ReadState(frame))
}
