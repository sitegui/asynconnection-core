...
function Peer(calls, messages, auth, connection) {
	...
	this._encoder.doHandshake(this.auth.user, password, calls.remote.list, messages.remote.list)
	this._localHandshake = Peer._HANDSHAKE_STATE.WAITING
}
...
Peer.prototype._processHandshake = function (data) {
	...
	if (this.auth.required && !(data.auth.user || data.auth.password)) {
		this._encoder.doAnswerHandshake(false)
		this._remoteHandshake = Peer._HANDSHAKE_STATE.DONE
	...
	this.auth.handler.call(this, data.auth.user, data.auth.password, function (err) {
...
Peer.prototype._processHandshakeAnswer = function (data) {
	...
	if (!data.ok) {
		this._error(new Error('Handshake failed'))
	} else if (this.handshakeDone) {
		this.emit('connect')
	}
}
