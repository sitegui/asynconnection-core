...
function Peer(calls, messages, auth, connection) {
	...
	this.auth = {
		...
		handler: auth.handler || function (auth, done) {
			done()
		}
	}
	...
	/**
	 * This is true after the handshake frame is received
	 * The handshake handler is called right after this
	 * Only after the handler returns the handShakeDone flag is turned on
	 * @member {boolean}
	 * @private
	 */
...
Peer.prototype._processHandshake = function (data) {
	...
	this.auth.handler.call(this, this.auth, function (err) {
		that.handshakeDone = true
		if (!err && !that.closed) {
			/**
